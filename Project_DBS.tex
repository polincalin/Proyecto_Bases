...
Combines the tables \texttt{Game}, \texttt{Map}, and \texttt{Episode} using \texttt{JOIN}, so each game session is linked to its map and episode. The \texttt{WHERE} clause keeps only finished sessions (those with \texttt{end\_time}). Then, \texttt{GROUP BY} organizes the data per map, and aggregate functions like \texttt{COUNT} and \texttt{AVG} are used to calculate how many sessions there are and their average duration.
...
\textbf{Interpretation.} This helps us see which maps tend to keep players playing for longer. For example, a map with longer sessions might be more engaging or take more time to complete.

\clearpage
\subsection{Query 2: players with the highest average proximity}
This query compares players by looking at their positions at the same \textit{tic}, even if they played in different sessions. It calculates the average distance between them.

This query compares players using a self-join, (\texttt{TelemetryEvent} joined with itself). 
The condition \texttt{a.tic = b.tic} matches positions at the same moment in time, and \texttt{a.player\_id < b.player\_id} avoids duplicating pairs. The distance between positions is calculated using \texttt{sqrt} and \texttt{power}, and then averaged using \texttt{AVG}. Finally, the results are grouped by pairs of players.
...
\textbf{Interpretation.} Since the game is single-player, this does not represent real interaction. Instead, we imagine all players moving at the same time.
...
This query uses a window function called \texttt{LAG()} to access the previous position of a player. The data is partitioned by \texttt{game\_id} and \texttt{player\_id}, so each player’s movement is analyzed within their own session. Then, the distance between consecutive positions is calculated and summed using \texttt{SUM}. Finally, \texttt{GROUP BY} is used to compute the minimum and maximum trajectory distance per player.
...
This query uses two common table expressions (CTEs). The first one counts how many tics each player has using \texttt{COUNT}. The second calculates the global average using \texttt{AVG}. Then, the query selects players above the average and joins several tables (\texttt{Player}, \texttt{UXResponse}, \texttt{UXItem}, and \texttt{UXSubscale}). Finally, it calculates the average score per subscale using \texttt{AVG}.
...
This query counts how many times each sector is visited using \texttt{COUNT(*)}. The data is grouped by episode, map, and sector. Then, the window function \texttt{ROW\_NUMBER()} is used to rank sectors based on the number of visits within each map. Finally, only the top-ranked sector (the most visited one) is selected.
...
\textbf{Interpretation.} Since the game is single-player, this does not represent real interaction. Instead, we imagine all players playing at the same time.
...
To ensure full reproducibility of the database, we provide a \texttt{Makefile} that automates the process of resetting and rebuilding the entire system from scratch. This guarantees that the database can be recreated consistently using only the SQL scripts and data files included in the repository.
...
\begin{itemize}
    ...
    \item \textbf{generate: }  
    This target executes the Python preprocessing script (\texttt{procesar.py}), which transforms the raw telemetry file (\texttt{Telemetry.tsv}) into a cleaned and structured TSV file (\texttt{telemetry.tsv}). The script also assigns metadata such as \texttt{game\_id}, \texttt{player\_id}, and \texttt{map\_id}, and computes derived fields such as sector assignment.

    \medskip

    \item \textbf{load: }  
    This is the main orchestration target responsible for fully rebuilding the system from scratch. It first executes \texttt{reset} and \texttt{generate}, and then runs all required SQL scripts in a strict order:

    \begin{itemize}
        \item The \textbf{DDL script}, which defines the complete database schema (tables, constraints, and relationships).
        \item The \textbf{ETL script (Part 2)}, which processes and validates staging data before insertion into the final tables.
        \item The \textbf{staging load step}, which imports the processed TSV file into \texttt{staging\_telemetry} using PostgreSQL's \texttt{\textbackslash copy} mechanism.
        \item The \textbf{UX data script (Part 3)}, which inserts the GUESS-18 instrument structure and survey data.
        \item The \textbf{C.3 views script}, which creates analytical views and materialized views for reporting.
        \item The \textbf{analytical queries script}, which defines queries used for evaluation and insights extraction.
        \item The \textbf{index evaluation script}, which creates and evaluates indexes for performance analysis.
    \end{itemize}
    ...
    \item \textbf{views: }  
    Recreates only the analytical views and materialized views defined in the C.3 section without resetting or modifying the underlying data. This is useful when the database already exists but the analytical layer needs to be refreshed or updated.

    \medskip

    \item \textbf{refresh: }  
    Recomputes the materialized view \texttt{player\_telemetry\_summary}, updating aggregated telemetry statistics based on the current data in the database.

    \medskip

    \item \textbf{analytics: }  
    Executes the full set of analytical SQL queries defined in \texttt{consultas\_analiticas.sql}. These queries are used to extract insights and validate the correctness of the data model.

    \medskip

    \item \textbf{indexes: }  
    Executes the index creation and evaluation script defined in \texttt{indices\_evaluacion.sql}, used to measure and demonstrate query performance improvements.

    \medskip

    \item \textbf{test: }  
    Runs a set of lightweight validation queries over key views. These queries provide a quick sanity check to verify that the database has been correctly built and that the main analytical outputs are available.
...
By integrating preprocessing (\texttt{generate}), schema creation, data ingestion through a staging layer, ETL processing, and analytical view construction, the Makefile ensures that the entire workflow can be reproduced end-to-end without ambiguity.
