...
In this section we present six analytical queries selected from the project brief. They were adapted to the
corrected schema, where a shared session is represented by game\_id and each player participation
is represented by (game\_id, player\_id).
...
A self-join compares players at the same game\_id and tic. A lower average Euclidean distance
...
This query uses LAG() to calculate step-by-step distance between consecutive tics, then aggregates
trajectory length by player. Using a window function provides a clearer solution than repeatedly
...
Trajectory duration is measured as the number of telemetry tics per player. The global average is
5,750 tics. Players 1, 2, and 3 are above that average, so their UX scores are grouped by subscale.
...
This query counts visits by episode, map, and sector, then uses ROW\_NUMBER() to keep the most
visited sector per map.
...
A self-join matches rows with the same game\_id, tic, and sector\_id, but different players. The
result counts how many tics each pair shared in the same sector.
